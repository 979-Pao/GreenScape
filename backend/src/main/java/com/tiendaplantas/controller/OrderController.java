...
public class OrderController {
  ...
  @GetMapping("/cart") public OrderDto myCart(Authentication auth){ return orders.getCart(auth.getName()); }

  @PreAuthorize("hasAnyRole('CLIENT','ADMIN','SUPPLIER')")
  @PostMapping("/cart/add") public OrderDto addToCart(Authentication auth, @RequestBody AddItemDto dto){
    return orders.addToCart(auth.getName(), dto.getPlantId(), dto.getQuantity());
  }

  @PreAuthorize("hasAnyRole('CLIENT','ADMIN','SUPPLIER')")
  @DeleteMapping("/cart/items/{itemId}") public OrderDto removeFromCart(Authentication auth, @PathVariable Long itemId){
    return orders.removeFromCart(auth.getName(), itemId);
  }

  @PreAuthorize("hasAnyRole('CLIENT','ADMIN','SUPPLIER')")
  @PostMapping("/cart/checkout") public OrderDto checkout(Authentication auth){ return orders.checkout(auth.getName()); }

  @PreAuthorize("hasRole('CLIENT')")
  @GetMapping("/cart/history") public List<OrderDto> myOrders(Authentication auth){ return orders.myOrders(auth.getName()); }

  // SUPPLIER: buzón y acciones
